...
class AppController extends Controller
{
    ...
    {		
		...
		//$files = $dir->find($this->request->getSession()->read('Auth.User.id').'.*', true);
		$files = $dir->find($this->request->getSession()->read('Auth.User.id').'\b.*', true);
		if ($files == null) {
		    $files = $dir->find('nulo\b.*', true);
		}
		$this->set('imagesUser', $files[0]);
